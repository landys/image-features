...
bool ImageFeature::load(const ::std::string& filename, bool forceGray){
  ...
  if(filename.find("gray") < filename.size() || filename.find(".pgm") < filename.size()) {
    forceGray=true;
  }
  ...
  try {
    ...
    ::std::cerr << "Exception loading image: " << filename << endl;
    return false;
  }
  ...
  if(loaded.colorSpace()==RGBColorspace && !forceGray) {
    ...
    ::std::cerr << "Unknown color space" << loaded.colorSpace() << endl; return false;
  }
  ...
  case 3: //rgb image
    ...
    break;
  default:
    ::std::cerr << "Unknown image format, depth=" << zsize_ << endl;
    return false;
    break;
  }
  
#else
  ::std::cerr << "Was compiled without imagemagick support. Image loading is not possible." << endl;
  return false;
...
void ImageFeature::display(const uint& idx1, const uint& idx2, const uint& idx3) const {
  ...
  toDisplay.display();
#else
  ::std::cerr << "Displaying images is not possible without ImageMagick" << endl;
#endif
}
...
void ImageFeature::save(const ::std::string & filename, const uint& idx1, const uint& idx2, const uint& idx3) {
  ...
  try {
    ...
    ::std::cerr << "Exception saving image: " << filename << endl;
    return;
  }
#else 
#warning "image saving is disabled without imagemagick"
  ::std::cerr << "Saving images is not possible without ImageMagick." << endl;
#endif
}
...
void ImageFeature::append(const ImageFeature& img) {
  if(img.xsize()==xsize_ && img.ysize() == ysize_) {
    ...
    ::std::cerr << "Images not compatible for appending" << endl;
  }
}
...
bool ImageFeature::read(::std:: istream & is) {
  ...
  if(line!="FIRE_imagefeature") {
    ::std::cerr << "Magicnumber not found. This is not a valid image feature" << endl;
    return false;
  } 
  
  getline(is,line); iss.clear(); iss.str(line); iss >> tmp;
  if(tmp!="sizes") {
    ::std::cerr << "Expecting 'sizes', got '" << line << "'." << endl;
    return false;
  } else {
  ...
  if(tmp=="filename") {  // filename is new in this structure
    ...
    ::std::cerr << "Expecting 'data', got '" << line << "'." << endl;
    return false;
  }
  return true;
}
