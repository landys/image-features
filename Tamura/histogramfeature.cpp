...
uint HistogramFeature::posToBin(const vector<uint>& pos) const {
  ...
  if(pos.size()!=steps_.size()) {
    std::cerr << "Invalid position: not right dimensionality." << endl;
    p=bins_.size()+1;
  } else {
    uint m=1;
    for(uint i=0;i<pos.size();++i) {
      if(pos[i]<steps_[i]) {
        ...
        std::cerr << "Invalid position: out of range." << endl;
        p=bins_.size()+1;
      }
    }
  }
...
/*void HistogramFeature::loadOld(const ::std::string &filename) {
  ...
  if(!is.good()) {
    std::cerr << "Unable to open histogram (old style)'"<< filename << "'."<< endl;
    return;
  } else {
    string line;
      ...
      if('#'==is.peek()) { // comment line
        ...
        if(!is.eof()) {
          ...
          if(token=="dim") {
            ...
            if(dim_!=min_.size()) {
              std::cerr << "min "<<min_.size() << "  and dim " << dim_ << " inconsistent while reading: " <<filename << endl;
            }
          } else if(token=="max") {
            ...
            if(dim_!=max_.size()) {
              std::cerr << "max " << max_.size() << " and dim " << dim_ << " inconsistent while reading: "<< filename << endl;
            }
            ...
            iss >> test;
            if(test != size_) {
              std::cerr << "nofBins " << test << " and size " << size_ << " inconsistent while reading: " <<filename<< endl;
            }
            ...
            for(unsigned int i=0;i<size_;++i) {
              ...
              is >> number >> content;
              if(number != i) {
                std::cerr << "While reading bins: number " << number << " != i " << i << " while reading: " << filename << endl;
              }
              bins_[i]=content;
              data_[i]=double(content)/double(counter_);
            }
          } else if (token!="") {
            std::cerr << "Unknown token in line: " << line << " while reading " << filename <<endl;
          } 
          iss.clear();
        }
      }
...
bool HistogramFeature::read(istream &is) {
  ...
  if(line!="FIRE_histogram") {
    std::cerr << "Not reading a valid Histogram:" << endl;
    return false;
  }
  ...
  if(token=="dim") {
    ...
    std::cerr << "Expected 'dim', got '" << line <<"'." << endl;
    return false;
  }
  ...
  if(token=="counter"){
    ...
    std::cerr << "Expected 'counter', got '" << token <<"'." << endl;
    return false;
  }

  if (counter_==0) { 
    counter_=1;
    std::cerr << "Histogram without data points. Setting counter_:=1 -> histogram has zero in all bins." << endl;
    return false;
  }
  ...
  if(token=="steps") {
    ...
    if(dim_!=steps_.size()) {
      ...
      //std::cerr << "steps " << steps_.size() << " and dim " << dim_ << " inconsistent." << endl;
    }
  } else {
    std::cerr << "Expected 'steps', got '" << line <<"'." << endl;
    return false;
  }
  ...
  if(token=="min") {
    ...
    if(dim_!=min_.size()) {
      std::cerr << "min "<<min_.size() << "  and dim " << dim_ << " inconsistent." << endl;
    }
  } else {
    std::cerr << "Expected 'min', got '" << line <<"'." << endl;
    return false;
  }
  ...
  if(token=="max") {
    ...
    if(dim_!=max_.size()) {
      std::cerr << "max " << max_.size() << " and dim " << dim_ << " inconsistent." << endl;
    }
  } else {
    std::cerr << "Expected 'max', got '" << line <<"'." << endl;
    return false;
  }
  ...
  if(token=="data"){
    ...
    if(size_!=bins_.size()) {
      ...
      //std::cerr << "data " << bins_.size() << " and size " << size_ << " inconsistent." << endl;
    }
  } else {
    std::cerr << "Expected 'data', got '" << line <<"'." << endl;
    return false;
  }
  this->initStepsize();
  iss.clear();
